...
describe('Location', () => {
  ...
  it('should contain the start and end line and column of exports', () => {
    ...
    const testClassLocation = new Location(moduleExports.find(e => e.name === 'TestClass')!.valueDeclaration!);
    expect(testClassLocation.start).toEqual({line: 0, character: 0});
    expect(testClassLocation.end).toEqual({line: 14, character: 1});

    const testFunctionLocation = new Location(moduleExports.find(e => e.name === 'testFunction')!.valueDeclaration!);
